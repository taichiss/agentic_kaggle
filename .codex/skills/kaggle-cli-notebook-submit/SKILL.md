---
name: kaggle-cli-notebook-submit
description: Kaggle CLI を使ってノートブックを bundle 化し、`kaggle kernels push` で提出する手順を扱う。`kernel-metadata.json` 生成、依存 source attach、push 後の status 確認、submission 生成確認が必要なときに使う。
---

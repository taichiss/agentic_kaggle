...
- PostToolUse フックは `.claude/hooks/post-tool-use.sh` から `uv run python scripts/quality_gate.py format` を呼ぶ。
- Kaggle 提出手順は `/kaggle-submit` コマンド（`.claude/commands/kaggle-submit.md`）で呼び出せる。
- `.codex/codex.md` は Codex/Antigravity 向けの同等ドキュメントとして並存する。
- `CLAUDE.md` が Claude Code の一次情報源であり、`.codex/codex.md` の内容を包含する。
- `.claude/settings.json` にパーミッションと PostToolUse フックが設定されている。
...
## 品質ハーネス
- 最速フィードバックは `PostToolUse` フックで取り、`bash .claude/hooks/post-tool-use.sh` から `uv run python scripts/quality_gate.py format` を呼ぶ（Claude Code 環境では `.claude/hooks/`、Codex 環境では `.codex/hooks/`）。
...
- ルールを変える場合は口頭で済ませず、`scripts/archgate.py` と `AGENTS.md` / `CLAUDE.md` を同時に更新する。
